...
(def next-turn {left-turn straight
                straight right-turn
...
(defn step-with-removal [track carts]
  (reduce (fn [new-carts cart]
...
(defn solve-2 []
  (let [{:keys [carts track]} (parse my-input)
        step' (partial step-with-removal track)]
    (->> carts
         (iterate step')
         (filter #(<= (count %) 1))
         first
...
(comment
  ...
  (let [{:keys [carts track]} (parse test-input2)
        step-with-removal' (partial step-with-removal track)]
    (println test-input2)
